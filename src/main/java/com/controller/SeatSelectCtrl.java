		...
		try {
			...
			if (classes[0] != 0 || classes[1] != 0 || classes[2] != 0) {
				...
				for (int i = 0; i < seatIds.size(); i++) {
					Seat seat = seatDao.getSeatById(seatIds.get(i), bookings.get(i).getFlightId());
					if (seat.getSeatStatus() == SeatStatus.UNAVAILABLE) {
						throw new DatabaseException("Cannot update seat information.");
					}
					int row2 = seatDao.alterSeatStatus(seat);
					//System.out.println(seat.getSeatStatus().toString());
					if (row2 <= 0) {
						throw new DatabaseException("Cannot update seat information.");
					}
				}

				// FlightSeat seat = new FlightSeat(flightId, businessLeft, firstLeft,
				// economyLeft, oldVersion++);

				FlightSeat seat = new FlightSeat(ticketsLeftVersion[0], ticketsLeftVersion[1] - counts[1],
						ticketsLeftVersion[2] - counts[0], ticketsLeftVersion[3] - counts[2], ticketsLeftVersion[4]++);
